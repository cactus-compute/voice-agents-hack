...
// LLM config pin. Runs on every service-worker boot.
//
// Default path: Google AI Studio (gemini-2.5-flash). User sets apiKey once
// via DevTools and it persists:
//   chrome.storage.local.set({apiKey: 'AIza…'})
//
// Experiment path: if storage already has provider='cactus', we leave the
// config alone so you can A/B between cloud and on-device by running:
//   chrome.storage.local.set({
//     provider: 'cactus',
//     apiBaseUrl: 'http://127.0.0.1:8765/v1/complete',
//     model: 'google/gemma-4-E2B-it',
//     maxTokens: 2048,
//   })
// …then reload. To return to Google, just:
//   chrome.storage.local.set({provider: 'google'})
// and reload.
chrome.storage.local.get(['provider', 'apiKey']).then(({ provider, apiKey }) => {
  if (provider === 'cactus') {
    console.log('[boot] LLM config left on Cactus (user override).');
    return chrome.storage.local.set({
      onboarding_completed: true,
      onboarding_version: 2,
    });
  }
  console.log('[boot] LLM config pinned to Google AI Studio (gemini-2.5-flash). ' +
              'Set apiKey via DevTools if not already set.');
  return chrome.storage.local.set({
    provider: 'google',
...
// Onboarding stripped — this build is driven via MCP, no UI flow needed.
// On install OR reload, force the LLM config to point at our local Cactus
// sidecar so the browser agent loop calls Cactus instead of any previously
