...
interface Session {
  ...
  url?: string;
  context?: string;
  status: "running" | "complete" | "error" | "stopped" | "waiting" | "awaiting_confirmation" | "timeout";
  steps: string[];
  answer?: string;
  error?: string;
  confirmation?: {
  ...
  userMessageQueue: AsyncQueue<string>;
  createdAt: number;
  updatedAt: number;
}
...
function formatResult(session: Session): any {
  ...
  if (session.confirmation) {
    result.confirmation = {
      ...
      payload: session.confirmation.payload,
    };
  }
...
function waitForSessionTerminal(sessionId: string, timeoutMs: number = TASK_TIMEOUT_MS): Promise<any> {
  return waitForRelayMessage(
    (msg) =>
      ...
      (msg.type === "task_complete" || msg.type === "task_error" || msg.type === "task_awaiting_confirmation"),
    timeoutMs
  );
}
    ...
    switch (name) {
      case "browser_start": {
        ...
        const session: Session = {
          ...
          status: "running",
          steps: [],
          userMessageQueue: new AsyncQueue<string>(),
          createdAt: Date.now(),
          updatedAt: Date.now(),
        };
      ...
      case "browser_message": {
        ...
        session.userMessageQueue.push(message);
