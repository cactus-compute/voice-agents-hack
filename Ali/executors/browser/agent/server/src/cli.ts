...
// --- Relay message handler (exported for testing) ---

export interface CliDeps {
  stderr: { write(s: string): void };
  stdin: { readLine(): Promise<string> };
  relay: { send(m: any): void };
}

export async function handleRelayMessage(message: any, deps: CliDeps): Promise<void> {
  ...
  switch (type) {
    case 'task_awaiting_confirmation': {
      ...
      if (payload && Object.keys(payload).length) {
        deps.stderr.write(`   ${JSON.stringify(payload, null, 2)}\n`);
      }
      deps.stderr.write(`   Respond ([y]es / [n]o / free text): `);
      const line = (await deps.stdin.readLine()).trim();
      const response =
        /^y(es)?$/i.test(line) ? 'yes, proceed' :
        /^n(o)?$/i.test(line) ? 'no, cancel' :
        line;
      deps.relay.send({
...
// Real stdin adapter using node:readline/promises
const realStdin: CliDeps['stdin'] = {
...
function handleMessage(message: any): void {
  ...
  const realDeps: CliDeps = {
    ...
    stdin: realStdin,
    relay: { send: (m: any) => connection.send(m) },
  };

  // handleRelayMessage is async; fire-and-forget is fine for non-confirmation events.
  // For task_awaiting_confirmation it will await stdin, which is intentional.
  handleRelayMessage(message, realDeps).catch((err) => {
    console.error('[CLI] handleRelayMessage error:', err);
  });
}
