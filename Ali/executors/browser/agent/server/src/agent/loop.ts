...
export interface AgentLoopParams {
  ...
  task: string;
  /** Optional session identifier — included in relay events */
  sessionId?: string;
  ...
  signal?: AbortSignal;
  /**
   * Optional relay — used to emit events (e.g. task_awaiting_confirmation) to
   * connected surfaces (MCP server, CLI, managed API). If omitted, relay events
   * are silently skipped.
   */
  relay?: { send(m: any): void };
  /**
   * Queue from which the loop reads the next user message when it needs to
   * pause for human input (e.g. after calling await_confirmation).
   * Required whenever the LLM may call await_confirmation — if the tool is
   * invoked and no queue is provided the loop throws immediately so the
   * misconfiguration is obvious rather than silently hanging.
   */
  userMessageQueue?: { next(): Promise<string> };
}
...
export async function runAgentLoop(
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    // Set to true when await_confirmation intercepts a tool_use and feeds the
    // user reply directly into `messages`. When true, skip the normal
    // toolResults push and turns.push at the bottom of the step.
    let awaitConfirmationHandled = false;
    for (const toolUse of toolUseBlocks) {
      ...
      // --- Special case: await_confirmation ---
      // Pause the loop until the user sends a message, then feed that message
      // back as the tool_result and re-enter the LLM turn (via `continue`).
      if (toolUse.name === "await_confirmation") {
        // Emit relay event so connected surfaces can render the confirmation UI
        if (relay) {
          relay.send({
            ...
            payload: toolUse.input.payload || {},
          });
        }

        // Block until the user sends a reply for this session
        if (!userMessageQueue) {
          throw new Error(
            "[AgentLoop] await_confirmation called but no userMessageQueue was provided. " +
            "Pass a userMessageQueue in AgentLoopParams so the loop can receive user replies."
          );
        }
        const userResponse = await userMessageQueue.next();

        // Feed the user's response back as a tool_result, then break out of the
        // toolUseBlocks loop and re-enter the outer step loop for the next LLM call.
        // We push to messages directly (not toolResults) so we can `break` immediately
        // — any remaining toolUseBlocks after await_confirmation are ignored, which is
        // correct: the LLM will re-plan once it sees the user reply.
        messages.push({
          role: "user",
          content: [{
            type: "tool_result",
        ...
        awaitConfirmationHandled = true;
        break; // exits the toolUseBlocks for-loop; outer for-loop continues to next step
      }
    ...
    // When await_confirmation intercepted the tool dispatch it already pushed
    // the user reply into `messages` and we skip the normal toolResults flush.
    if (!awaitConfirmationHandled) {
